...
//
function check(/*atom*/ n, /*string*/ rtn) {
    if (compare(n,1)<0 || (n!==floor(n))) {
        crash("argument to %s() must be a positive integer",["sequence",rtn]);
    } else if (compare(n,power(2,((machine_bits()===32) ? 53 : 64)))>0) {
...
//  if duplicates is true returns a true decomposition of n (eg 8 --> {2,2,2})
    ...
    while (compare(p,lim)<=0) {
        if (remainder(n,p)===0) {
            ...
            while (1) {
                n = n/p;
                if (remainder(n,p)!==0) { break; }
                if (duplicates) {
                    pfactors = append(pfactors,p);
                }
            }
            if (compare(n,p)<=0) { break; }
            lim = min(floor(sqrt(n)),get_prime(maxprime));
        }
        pn += 1;
        p = get_prime(pn);
    }
    if (compare(n,1)>0 && ((length(pfactors)!==0) || duplicates)) {
...
//  (but terminating early and without building any unnecessary internal lists)
    ...
    while (compare(p,lim)<=0) {
        if (remainder(n,p)===0) {
            n = n/p;
            if (remainder(n,p)===0) { return false; }
            if (compare(n,p)<=0) { break; }
            lim = min(floor(sqrt(n)),get_prime(maxprime));
        }
        pn += 1;
