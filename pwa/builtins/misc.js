...
//integer {from_dx,to_dx} = from_to, len = length(src)
    let /*integer*/ from_dx = $subse(from_to,1), to_dx = $subse(from_to,2), len = length(src);
    let /*integer*/ uppr, midpoint;
    let /*sequence*/ res;
    if (compare(from_dx,0)<0) {
        from_dx = len+1+from_dx;
    }
    if (compare(to_dx,0)<0) {
        to_dx = len+1+to_dx;
    }
    if (compare(len,2)<0 || (from_dx===to_dx)) {
        return src;
    }
