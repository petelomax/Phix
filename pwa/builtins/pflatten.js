...
/*global*/ function flatten(/*sequence*/ s, res="") {
    for (let i=1, i$lim=length(s); i<=i$lim; i+=1) {
        ...
        if (atom(si) || string(si)) {
            ...
            res = $conCat(res, flatten(si), false);
        }
    }
    return res;
} flatten.$sig="FPP,3";

/*global*/ function join(/*sequence*/ s, /*object*/ delim=0X20, lastdelim=delim, /*string*/ fmt="") {
    let /*sequence*/ res = "";
    let /*integer*/ ls = length(s), lf = length(fmt);
    for (let i = 1, si$lim = length(s); i <= si$lim; i += 1) { let si = $subse(s,i);
        if (i!==1) {
            if (i===ls) { delim = lastdelim; }
            res = $conCat(res, delim, false);
        }
        if (lf) { si = sprintf(fmt,si); }
        res = $conCat(res, si, false);
    }
    return res;
} join.$sig="FPOOS,1";
...
//  s = iff(length(fmt)?apply(true,sprintf,{{fmt},s}):deep_copy(s))
    s = deep_copy(s);
    if (length(fmt)) {
        for (let i=skip+1, i$lim=ls; i<=i$lim; i+=1) {
            s = $repe(s,i,sprintf(fmt,$subse(s,i)));
        }
    }
...
//      if step!=1 or length(s)=0 then js &= n_pad end if
        if (((step!==1) || (equal(length(s),0))) || (equal(length(step_pad),0))) { js = $conCat(js, n_pad, false); }
        res = append(res,js);
    }
    if (length(s)) {
//      js = join(s,n_pad)
...
//integer SLASH = iff(platform()=WINDOWS?'\\':'/')
    ...
    for (let i=1, i$lim=length(path_elements); i<=i$lim; i+=1) {
        ...
        fname = $conCat(fname, elem, false);
    }
    if ((trailsep && length(fname)) && (!equal($subse(fname,-1),SLASH))) {
        fname = $conCat(fname, SLASH, false);
    }
    return fname;
} join_path.$sig="FPI,1";
