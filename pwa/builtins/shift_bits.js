...
/*global*/ function shift_bits(/*object*/ source_number, /*integer*/ shift_distance) {
    let /*integer*/ lSigned;
    if (sequence(source_number)) {
        for (let i=1, i$lim=length(source_number); i<=i$lim; i+=1) {
            source_number = $repe(source_number,i,shift_bits($subse(source_number,i),shift_distance));
        }
    ...
    if (shift_distance<0) {
        ...
        if (and_bits(source_number,0x80000000)) {
            lSigned = 1;
