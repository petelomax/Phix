    ...
    if (sequence(x)) {
        for (let i=1, i$lim=length(x); i<=i$lim; i+=1) {
            ...
            if (sequence(o)) {
                ...
                c = o;
                if (compare(c,0)>0 && compare(c,255)<=0) {
...
/*global*/ function isupper(/*integer*/ ch) {
    if (!cinit) { initcase(); }
    return compare(ch,0)>0 && compare(ch,255)<=0 && (ch!==$subse(toLower,ch));
}
/*global*/ function islower(/*integer*/ ch) {
    if (!cinit) { initcase(); }
    return compare(ch,0)>0 && compare(ch,255)<=0 && (ch!==$subse(toUpper,ch));
}
//DEV doc/auto:
//**
...
--end function
*/
const str_methods = ["sequence","LOWER", "UPPER", "CAPITALISE", "SENTENCE", "INVERT"];
...
//      ishigh := (ch>='A' and ch<='Z') highit = false
        islow = compare(ch,0)>0 && compare(ch,255)<=0 && (ch!==$subse(toUpper,ch)); lowit = false;
        ishigh = compare(ch,0)>0 && compare(ch,255)<=0 && (ch!==$subse(toLower,ch)); highit = false;
        switch (mi) {
