...
//end if
    ...
    if ($subse(pline,plen)===0XA) {
        spurge();
        if ((ppp_File===1) && ppp_Pause) {
            sline += 1+(compare(plen,ppp_Maxlen)>0);
            if (compare(sline,ppp_Pause)>=0) {
/**/
                if (wait_key()) { } /*    -- Phix
...
//      if ppp_StrFmt<=0 then
        if (string(cl) && compare(ppp_StrFmt,0)<=0) {
//put back 26/4/2020:
...
//              if find(chint,escBytes) then    -- "\t\n\r\\\"\'"
                ...
                if (k) {
                    aschar += 1;
                    if (compare(k,3)<0) { asbacktick = false; }
                    if (ppp_Q22 && (chint===0x22)) { // ie '"'
                        // \"blah ==> #22&"blah
...
//              len = length(cl)+(aschar-1) -- allow for escape chars
                ...
                if (prnt) {
                    if (compare(aschar,1)>0 && !asbacktick) {
...
//              and ppp_StrFmt!=-3 then
        ...
        if (length(ppp_Date) && (length(cl)===3) && integer($subse(cl,1)) && integer($subse(cl,2)) && integer($subse(cl,3)) && ((compare($subse(cl,1),1)>=0 && compare($subse(cl,1),31)<=0 && compare($subse(cl,3),1900)>=0 && compare($subse(cl,3),3000)<=0) || (compare($subse(cl,3),1)>=0 && compare($subse(cl,3),31)<=0 && compare($subse(cl,1),1900)>=0 && compare($subse(cl,1),3000)<=0)) && (compare($subse(cl,2),1)>=0 && compare($subse(cl,2),12)<=0)) {
        ...
        if (compare(nestlvl,0)<0) { nestlvl = 0; }
        sep = "";
        for (let i=1, i$lim=length(cl); i<=i$lim; i+=1) {
            if (compare(col+len,nindent*($subse(ppp_Nest,2)+1-nestlvl))>0) {
                if ((compare(i,1)>0 && compare(nestlvl,0)>0) || compare(ppp_Maxlen,prnf($subse(cl,i),col+len,col+nindent,0,nestlvl-1)+col+len+length(sep)+2+ppp_Indent)<=0) {
            ...
            iplus1 = i+1;
            if (atom($subse(cl,i)) && ((i===length(cl)) || atom($subse(cl,iplus1)))) {
                // as req by Juergen Luethje
                sep = ",";
            } else {
                sep = ", ";
    ...
    if (integer(cl) || (compare(cl,-0xFFFFFFFF)>=0 && compare(cl,0xFFFFFFFF)<=0 && (cl===floor(cl)))) {
...
//3/12/18:
//      elsif cl<0 then
        } else if (compare(cl,0)<0 && ($subse(ppp_IntFmt,1)!==0X25)) {
...
//      elsif cl<0 and ppp_FltFmt[2]='.' then
        } else if (compare(cl,0)<0 && ($subse(ppp_FltFmt,1)!==0X25)) {
...
/*with warning*/ //procedure fatal(sequence msg)
...
function fatal(/*string*/ msg, /*integer*/ level) {
//  puts(1,msg)
//  warned = true
//18/12/20:
 /*
...
//  crash(msg,nFrames:=level)
    crash(msg,["sequence"],level); // pwa/p2js: JavaScript does not support named parameters...
}
function setAscii() {
    ascii = repeat(0,255);
    if (length(ppp_Ascii)!==2) { fatal("length(ascii) must be 2",4); }
    let /*object*/ [,minasc, maxasc] = ppp_Ascii;
    if (!sequence(minasc)) { minasc = ["sequence",minasc]; }
    if (!sequence(maxasc)) { maxasc = ["sequence",maxasc]; }
    if (!equal(length(minasc),length(maxasc))) { fatal("length(minasc)!=length(maxasc)",4); }
    if (find(0,minasc) || find(0,maxasc)) { fatal("find(0,minasc) or find(0,maxasc)",4); }
...
function setOpt(/*sequence*/ options) {
    ...
    let /*object*/ tmp;
    if (!ppp_Init) { pp_Init(); }
    if (and_bits(1,length(options))) { fatal("length(options) not even",flvl); }
