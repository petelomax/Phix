...
/*global*/ function $keyvalues(/*sequence*/ source, /*object*/ pair_delim=";,", /*object*/ kv_delim=":=", /*object*/ quotes="\"'`", /*object*/ whitespace=["sequence",0X20, 0X9, 0XA, 0XD], /*integer*/ haskeys=1) {
    let /*sequence*/ lEndBracket;
    let /*sequence*/ lBracketed;
    source = trim(source);
    ...
    while (compare(sdx,length(source))<=0) {
        ...
        while (compare(sdx,length(source))<0) {
            if (equal(find($subse(source,sdx),whitespace),0)) {
                break;
            }
            sdx += 1;
        }
        ...
        if ((!equal(find(ch,kv_delim),0)) || !haskeys) {
            ...
            while (compare(sdx,length(source))<=0) {
                ...
                if ((equal(length(lBracketed),0)) && (!equal(find(ch,quotes),0))) {
                    ...
                    lBracketed = $conCat(lBracketed, $subse("}])",bpos));
                ...
                if (compare(length(argtext),0)>0) {
                                ...
                                if (pos) {
                                    if (decs===0) {
                                        ...
                                        if (pos>decs) {
                                            ...
                                            argtext = $conCat(argtext, repeat(0X30,decs-pos));
                                        }
                                    }
                                } else if (decs>0) {
                    ...
                    if (compare(width,length(argtext))<0) {
                        ...
                        if (align===0X3E) {
                            ...
                            argtext = $conCat(argtext, repeat(0X20,width-length(argtext)));
                        }
                    }
                    result = $conCat(result, argtext);
                } else {
                    if (spacer) {
                        result = $conCat(result, 0X20);
                    }
                }
                if (trimming) {
                    result = trim(result);
