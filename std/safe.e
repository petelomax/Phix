...
--DEV PL
--override 
procedure poke2(atom a, object v)
...
-- Check all allocated blocks for corruption of the leader and trailer areas. 
        ...
        for i = 1 to length(safe_address_list) do
                ...
                if block[3] >= 1 then
                        ...
                        a = block[1]
                        n = block[2]
                        if not equal(leader, peek({a-BORDER_SPACE, BORDER_SPACE})) then
                                show_block(block)
                        elsif not equal(trailer, peek({a+n, BORDER_SPACE})) then
                                show_block(block)
                        end if
                end if
        end for
...
public procedure free_code( atom addr, integer size, valid_wordsize wordsize = 1 )
        ...
        for i = 1 to length(safe_address_list) do
                ...
                if block[BLOCK_ADDRESS] = addr then
                        ...
                        if not equal(leader, peek({addr-BORDER_SPACE, BORDER_SPACE})) then
                                show_block(safe_address_list[i])
                        elsif not equal(trailer, peek({addr+n, BORDER_SPACE})) then
                                show_block(safe_address_list[i])
                        end if
                        safe_address_list = remove( safe_address_list, i )
                        exit
