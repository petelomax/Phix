    ...
    for i=1 to length(files) do
        if not find(files[i][D_NAME], {".", ".."}) then
            if find('d', files[i][D_ATTRIBUTES]) then
                if recurse then
                    cnt = clear_directory(path & files[i][D_NAME], recurse)
                    if cnt=0 then
                        return 0
                    end if
                    ret += cnt
...
ifdef not UNIX then
    ...
    if length(lPath)=0 or lPath[1]!=SLASH then
ifdef UNIX then
        lPath = curdir() & lPath
elsedef
...
ifdef WIN32 then
    pName = allocate_string(name)
    r = c_func(xGetFileAttributes, {pName})
    free(pName)
...
elsifdef UNIX then
    pName = allocate_string(name)
    r = c_func(xGetFileAttributes, {pName, 0})
    free(pName)
...
ifdef WIN32 then
    ...
    success = c_func(xCopyFile, {psrc, pdest, not overwrite})
    free(pdest)
    free(psrc)
...
ifdef UNIX then
    ...
    if overwrite then
        ...
        if delete_file(tempfile) then end if
    end if
...
--DEV (ifdef implemented differently in Phix!)
ifdef UNIX then
...
elsedef
    ...
    if overwrite then
        ...
        tempfile = temp_file(dest)
        if move_file(dest, tempfile) then end if
    end if
    ...
    if overwrite then
        if not ret then
            ...
            if move_file(tempfile, dest) then end if
        end if
        if delete_file(tempfile) then end if
    end if
