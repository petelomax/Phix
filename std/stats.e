...
--/* Not Phix: but note that builtins\psmall.e (see that for details) does not have the "only atoms" thing.
--**
...
function massage(sequence data_set, object subseq_opt)
    if subseq_opt=ST_IGNSTR then
        return remove_subseq(data_set, SEQ_NOALT)
    elsif subseq_opt=ST_ZEROSTR then
...
public function count_atoms(object data_set, object subseq_opt = ST_ALLNUM)
    if atom(data_set) then
        return 1
    end if
...
--/* see demo\rosetta\Average_Median.exw
...
--/* see demo\rosetta\Averages_Mode.exw
