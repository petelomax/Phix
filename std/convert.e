...
integer ti
    ...
    for i=1 to length(text) do
        ...
        if    ti='_' then   -- ignore
            ...
            if div!=0 then exit end if
            div = 1
        elsif ti='-' then
            if sign!=0 or n!=0 then exit end if
            sign = -1
        else
...
integer lChar
    ...
    for i=1 to length(text_in) do
        ...
        lChar = text_in[i]
--DEV temp:: (but would be interesting to compare the speed on Eu4 vs Phix)
--/**/  if lChar='-' then
--/*
        switch lChar do
            case '-' then
...
--/**/  elsif (lChar>='0' and lChar<='9')
--/**/     or (lChar>='A' and lChar<='F')
--/**/     or (lChar>='a' and lChar<='f') then
--/*
