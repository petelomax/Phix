...
integer hi, ci
    ...
    printf(fn, "Database dump as at %s\n", {datetime:dateformat(now(), "%Y-%m-%d %H:%M:%S")})
    safe_seek(0)
...
public function db_create(sequence path, integer lock_method = DB_LOCK_NO,
                          integer init_tables = INIT_TABLES, integer init_free = INIT_FREE)
integer db
...
-- Any lock will be removed, allowing other processes to access the database file. 
-- The current database becomes undefined.
...
--
-- Errors:
--  If the current table is not defined, or ##key_location## is not a valid record index, an error will occur. 
--  Valid record indexes are between 1 and the number of records in the table.
--
-- Example 1:
-- <eucode>
-- db_delete_record(55)
