...
--              else
                    IupMessage("Warning",sprintf("No translation for %s in %s",{text,tffile}))
--              end if
                firsterror=0
            end if
        else
...
procedure Xerr(sequence msg)
    if firsterror then
        IupMessage("Translation file error",sprintf("%s line %d: %s",{tfname,line,msg}))
        firsterror = 0
        if seek(tf,-1)!=SEEK_OK then ?9/0 end if
    end if
end procedure
...
constant LOCALE_USER_DEFAULT = #400,
         LOCALE_SABBREVLANGNAME = 3

function getLocale()
string res
integer k
atom kernel32, xGetLocaleInfo, pBuf
    if platform()=WINDOWS then
        pBuf = allocate(4)
        kernel32 = open_dll("kernel32.dll")
        xGetLocaleInfo = define_c_func(kernel32, "GetLocaleInfoA",
                                        {C_LONG,    --  LCID  Locale,       // locale identifier 
                                         C_LONG,    --  LCTYPE  LCType,     // type of information 
                                         C_PTR,     --  LPTSTR  lpLCData,   // address of buffer for information 
                                         C_INT},    --  int  cchData         // size of buffer 
