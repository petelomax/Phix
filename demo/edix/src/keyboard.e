...
Listview with Edit/double click
textual: (A-Z), F1-F12, Insert etc, Ctrl/Alt/Shift
(The Ctrl L can be overidden in Options/Keyboard - DOC)
softlock: grey out some things by default, but allow overrides if some checkbox is set.
Multiple settings (eg Paste: Ctrl V,Shift Insert) [and Alt I: Invert Case|&Idit Menu??]
xlate as input to the display...
Unused Keys
Conflicts
Movement | Ctrl/Alt | Function | Find | Menu

easinst.ew -> singinst.ew [??Windows only??]

This will almost certainly require that we add K_cA etc (quite a few) to pGUI.e..

*** Store settings in a text file ***
-- (so I can distribute a standard setting and users can copy theirs about)
-- (use 2 or 3 files so re-install does not clobber the current settings)
--SUG: keys.txt: not read*, but explains the standard key settings, and     (eg Open: Ctrl O)
--     kmap.txt: shows the (barely readable) mapping to keys.txt            (eg Ctrl L->Ctrl O)
--      Partial and even empty kmap.txt are fine - the only rules are:
--          All LHS must be unique.
--          All RHS must exist in keys.txt.
--          A blank RHS makes the LHS inoperative.
--      Conflict errors and reload are the responsibility of this program.
--     *keys.txt would in fact be read by this program, I meant not as part 
--      of startup, or for normal operation of edix. Note that the values 
--      in keys.txt are all as permanently hard-coded in edix.exw (etc).
--
-- keys.txt
-- ========
--
...
--
--  This file serves to document the keys permanently hardcoded into edix.
--  The only times this file should be changed are when the sources of
--  edix are altered to use different standard keys, or when running the
--  Options/Keyboard Settings of edix has obvious errors/typos showing,
--  or to specify/alter standard mappings for when no kmap.txt exists.
--  Apart from that very last bit, and some of the text shown when 
--  Options/Keyboard Settings is run, changing this file will not alter 
--  the way anything works. This file will be overwritten when a new
--  version of edix is installed, whereas kmap.txt will be left alone.
--
--  The separate file kmap.txt contains key mappings, which translate the
--  keys pressed into something the internals will understand.
