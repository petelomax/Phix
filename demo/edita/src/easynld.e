...
--      void = messageBox(sprintf("Error: %s line %d",{fullname,lineno}),msg,0)
        errorMsg = {sprintf("Error: %s line %d",{fullname,lineno}),msg}
    end if
    errorAlready = 1
...
function Expect(sequence txt)
    ...
    for i=1 to length(txt) do
        if ch!=txt[i] then 
            if UpperCase(ch)!=UpperCase(txt[i]) then
    ...
    if sequence(dirlist) then
        for i=1 to length(dirlist) do
            if not find('d',dirlist[i][2]) then
                ...
                len = length(filename)
                if len>4 
                and equal(".syn",LowerCase(filename[len-3..len])) then
...
--end if
    ...
    if length(errorMsg) then
        void = messageBox(errorMsg[1],errorMsg[2],0)
    end if
end procedure
initSyn()
    ...
    if sequence(dirlist) then
        ...
        for i=1 to length(dirlist) do
            if not find('d',dirlist[i][2]) then
                ...
                and equal(".txt",LowerCase(filename[len-3..len]))
                and not equal("readme.txt",LowerCase(filename)) then
                    k += 1
                    dirlist[k] = filename
                end if
            end if
...
integer oldColour
    for i=1 to length(cnew) do
        if cnew[i]!=cold[i] then
            ...
            if oldBrush then void = c_func(xDeleteObject,{oldBrush}) end if
            oldColour = cold[i]
...
--          elsif i=Other then
                if oldBrush then void = c_func(xDeleteObject,{wwrapPen}) end if
                wwrapPen = c_func(xCreatePen,{2,1,not_bits(oldColour)})
                void = c_func(xSelectObject,{mainDC,wwrapPen})
            end if
            cnew[i] = oldColour
        end if
    end for
