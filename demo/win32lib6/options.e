...
--/i pCategory is the name of a category, or grouping, found in /i pSource. The catgories are coded
-- in the file in the form "[" category "]" as the only thing in a line. /n
...
global function getOpt(sequence pSource, sequence pCategory, sequence pKey, object pDefault)
            ...
            if lInCategory then
                ...
                if lPos then 
                    ...
                    if equal(lKey, pKey) then
                        lOptData = w32trim(lData[lPos+1..length(lData)])
    ...
    close(lFH)
    if length(lOptData) >= 2 and lOptData[1] = '"' and lOptData[length(lOptData)] = '"' then
