...
--  getting mixed errors of LDAP_SERVER_DOWN/LDAP_INVALID_CREDENTIALS/LDAP_INVALID_DN_SYNTAX
--  and LDAP_CONFIDENTIALITY_REQUIRED.
--  See https://rosettacode.org/wiki/Active_Directory/Connect#Phix
--  and https://rosettacode.org/wiki/Active_Directory/Search_for_a_user#Phix
--
--  Give me a shout if you need any help or advice with this. I do not plan to do any more
--  work on this unless I know someone is using it, or at least wants to. (I half-planned
--  to do something with c_structs, but it didn't really pan out.)
--  
include builtins\cffi.e

constant wldap32 = open_dll("wldap32.dll"),
t_ldap_init = """
PVOID ldap_init(
  PSTR  HostName,
  ULONG PortNumber
);""",
x_ldap_init = define_cffi_func(wldap32,t_ldap_init),
LDAP_PORT = 389,
...
x_ldap_simple_bind_s = define_cffi_func(wldap32,t_ldap_simple_bind_s),
t_ldap_search_s = """
ULONG ldap_search_s(
  LDAP         *ld,
  PSTR         base,
...
x_ldap_search_s = define_cffi_func(wldap32,t_ldap_search_s),
t_ldap_msgfree = """
ULONG ldap_msgfree(
  LDAPMessage *res
);""",
x_ldap_msgfree = define_cffi_func(wldap32,t_ldap_msgfree),
t_ldap_unbind = """
ULONG WINAPI ldap_unbind(
  LDAP *ld
);""",
x_ldap_unbind = define_cffi_func(wldap32,t_ldap_unbind)

global constant LDAP_SCOPE_BASE     = 0x0000,
...
constant epairs = {{LDAP_SUCCESS,                   "LDAP_SUCCESS"},                    -- The call completed successfully.
                   {LDAP_SERVER_DOWN,               "LDAP_SERVER_DOWN"},                -- Cannot contact the LDAP server.
                   {LDAP_INVALID_CREDENTIALS,       "LDAP_INVALID_CREDENTIALS"},        -- The supplied credential is invalid.
                   {LDAP_CONFIDENTIALITY_REQUIRED,  "LDAP_CONFIDENTIALITY_REQUIRED"},   -- Confidentiality is required.
                   {LDAP_INVALID_DN_SYNTAX,         "LDAP_INVALID_DN_SYNTAX"}},         -- The distinguished name has an invalid syntax.,
         {ecodes,edesc} = columnize(epairs)

global function ldap_init(string host, integer port=LDAP_PORT)
atom ld = c_func(x_ldap_init,{host,port})
    return ld
end function

global function ldap_simple_bind_s(atom ld, string dn, passwd)
    atom res = c_func(x_ldap_simple_bind_s,{ld,dn,passwd})
    if not find(res,ecodes) then ?9/0 end if
    return res
end function

global function ldap_search_s(atom ld, string base, integer scope, string filter, atom pAttrs, bool attrsonly)
    atom ppMsg = allocate_word(NULL,true)
    integer res = c_func(x_ldap_search_s,{ld,base,scope,filter,pAttrs,attrsonly,ppMsg})
    if not find(res,ecodes) then ?9/0 end if
    return {res,peekns(ppMsg)}
end function

global procedure ldap_msgfree(atom pMsg)
    if pMsg!=NULL then
        integer res = c_func(x_ldap_msgfree,{pMsg})
        if res!=LDAP_SUCCESS then ?9/0 end if
    end if
end procedure

global procedure ldap_unbind(atom ld)
    atom res = c_func(x_ldap_unbind,{ld})
    if res!=LDAP_SUCCESS then ?9/0 end if
end procedure

global function ldap_err_desc(integer res)
    integer k = find(res,ecodes)
    string desc = edesc[k]
    return desc
end function

-- The rest of this file is just text copied from mny searches, that may or may not be useful...
--from https://docs.microsoft.com/en-us/previous-versions/windows/desktop/ldap/functions
--erm, https://docs.microsoft.com/en-us/windows/win32/api/_ldap/
...
--/*
"ldap.bu.edu",
--"ldap.baylor.edu",    -- LDAP_SERVER_DOWN
"ldap.andrew.cmu.edu",
--"directory.colostate.edu",    -- LDAP_SERVER_DOWN
"directory.d-trust.de",
"x500.bund.de",
--"pksldap.tttc.de",        -- LDAP_SERVER_DOWN
--"ldap-tiks.telesec.de",   -- LDAP_SERVER_DOWN
"ldap.telesec.de 389",
--"www.trustcenter.de",     -- LDAP_SERVER_DOWN
--"directory.verisign.com", -- LDAP_SERVER_DOWN
"db.debian.org",
"ldap.itd.umich.edu",
--"ldap.virginia.edu",      -- LDAP_SERVER_DOWN
"ldap.duke.edu",
--*/
