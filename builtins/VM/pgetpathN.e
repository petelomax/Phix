...
--forward global function allocate(integer size)    --DEV?? (implicit->global vs init code warning..)
--include builtins\VM\pAlloc.e      -- allocate() [DEV move to pHeap?]
include builtins\VM\pprntfN.e
include builtins\platform.e
include builtins\VM\pcfuncN.e
include builtins\VM\pcurrdirN.e [DEAD]
...
atom kernel32, xGetLongPathName, buffer
...
pp(filepath)
--/**/  if not c_func(xGetLongPathName,{filepath,buffer,MAX_PATH}) then ?0 poke(buffer,0) end if        --/* -- Phix
        pFilePath = allocate_string(filepath)                                                       -- RDS
        if not c_func(xGetLongPathName,{pFilePath,buffer,MAX_PATH}) then poke(buffer,0) end if      -- RDS
        free(pFilePath)                                                                             --*/ -- RDS
