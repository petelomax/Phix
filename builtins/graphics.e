...
--  Phix compatible version (effectively empty except for include pscreen.e)
...
constant LINUX = 3
--*/
--/**/  include pscreen.e   -- video_config()
--/**/ --(the above allows eg
--/**/ --   include graphics.e as graphics
--/**/ --   ?graphics:video_config()
--/**/ -- to work equally on both Phix and RDS Eu)
...
constant
     M_SOUND          = 1,      -- DOS only
     M_LINE           = 2,      -- DOS only
     M_PALETTE        = 3,      -- DOS only
     M_GRAPHICS_MODE  = 5,      -- DOS only
     M_CURSOR         = 6,
     M_WRAP           = 7,      -- to be made a builtin
     M_SCROLL         = 8,      -- to be made a builtin
     M_SET_T_COLOR    = 9,      -- opTxtClr
     M_SET_B_COLOR    = 10,     -- opBkClr
     M_POLYGON        = 11,     -- DOS only
     M_TEXTROWS       = 12,     -- to be made a builtin
     M_VIDEO_CONFIG   = 13,     -- (see pscreen.e)
     M_ELLIPSE        = 18,     -- DOS only
     M_GET_POSITION   = 25,     -- opGetPos
     M_ALL_PALETTE    = 27      -- DOS only
--*/
...
global
procedure draw_line(color c, point_sequence xyarray)
...
--/* Not required for Phix (DOS only)
global
procedure polygon(color c, boolean fill, point_sequence xyarray)
...
--/* Not required for Phix (DOS only)
global
procedure ellipse(color c, boolean fill, point p1, point p2)
...
--/* Not Phix (these are defined in psym.e)
...
--/* Not required for Phix (defined as opGetPos) [DEV in pfileioN.e] 
global
...
global
function text_rows(positive_int rows)
    return machine_func(M_TEXTROWS, rows)
end function

global
procedure wrap(boolean on)
...
global
procedure scroll(integer amount, 
...
--/* Not required for Phix (DOS only)
global
function palette(color c, mixture s)
...
--/* Not required for Phix (DOS only)
global
procedure all_palette(sequence s)
...
--/* Not required for Phix (DOS only) [DEV use Beep()?]
type frequency(integer x)
    return x >= 0
end type

global
procedure sound(frequency f)
