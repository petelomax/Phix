...
function toString(sequence s)
string res
atom pRes
    res = repeat(' ',length(s)*4)
    #ilASM{ mov eax,[res]
            push ebx
            shl eax,2
            push eax
...
--          fld qword[esp]
            ...
            lea edi,[pRes]
--          lea edx,[pRes]
            call %opMovbi } -- opStoreFlt
    poke4(pRes,s)
    return res
end function

function sbox(object o)
atom hash = 0
    if atom(o) then
        if integer(o) then
            #ilASM{ lea esi,[o]
...
global function sbox_hash(object o)
    if pSbox = 0 then
        pSbox = allocate(256*4)
        poke4(pSbox,Sbox)
    end if
    return sbox(o)
end function

global function jenkins32_hash(object o)
atom hash = 0
    if atom(o) then
        if integer(o) then
            #ilASM{ lea esi,[o]
...
--/*
DIGEST_TYPE FletcherHash (char *key, unsigned int len)
{
...
--/*
DIGEST_TYPE AdlerHash (char *str, unsigned int len)
{
