--
-- Phix compatible version of/replacement for image.e
--
...
--/**/  include pscreen.e   -- get_screen_char, put_screen_char,
--                          -- display_text_image, and save_text_image.
...
integer Type, Xhot, Yhot, Planes, BitCount, Width, Height,
        SizeHeader, Compression, NumColors
...
--/* Phix version is now in pscreen.e
...
global
procedure display_image(graphics_point xy, sequence pixels)
...
--/* Not required for Phix (DOS only)
global
function save_image(graphics_point top_left, graphics_point bottom_right)
...
--/* Not required for Phix (DOS only)
global
procedure set_display_page(page_number page)
...
--/* Not required for Phix (DOS only)
global
procedure set_active_page(page_number page)
...
integer page_size
    ...
    return screen_memory + ((xy[1]-1) * vc[VC_COLUMNS] + (xy[2]-1)) * BYTES_PER_CHAR
end function
--*/

--/* -- (Phix implementation is now in pscreen.e)
global
function get_screen_char(positive_atom line, positive_atom column)
...
--/* -- (Phix implementation is now in pscreen.e)
global
procedure put_screen_char(positive_atom line, positive_atom column, sequence char_attr)
...
--/* -- (Phix implementation is now in pscreen.e)
global
procedure display_text_image(text_point xy, sequence text)
...
--/* -- (Phix implementation is now in pscreen.e)
global
function save_text_image(text_point top_left, text_point bottom_right)
...
--          row_chars &= get_screen_char(row, col)
        end for
    end if
...
integer numXPixels
        numXPixels = 0
integer numYPixels, bitCount, numRowBytes
--/* Not required for Phix
...
integer  j, byte, numBytesFilled, jplusk
...
sequence pali
    ...
    for i=1 to numColors do
        pali = pal[i]
        puts(fn, pali[3])   -- blue first in .BMP file
        puts(fn, pali[2])   -- green second 
        puts(fn, pali[1])   -- red third
...
--/* Not required for Phix (DOS only)
global
function save_screen(region r, sequence file_name)
