...
--/* Not Phix (defined in autoinclude builtins\pcolumn.e)
--**
...
--DEV Phix: defined in misc.e, may want extending to match this.
--/*
public function reverse(object target, integer pFrom = 1, integer pTo = 0)
    ...
    integer lLimit = floor((pFrom+pTo-1)/2),
              uppr = pTo
    sequence t = deep_copy(target)
...
public function linear(object start, object increment, integer count)
    ...
    sequence result = repeat(start,count)
    for i=2 to count do
        start += increment
        result[i] = start
    end for
...
public function add_item(object needle, sequence haystack, integer pOrder = 1)
    ...
    if pOrder=ADD_PREPEND then

        return prepend(haystack, needle)

    elsif pOrder=ADD_APPEND then

        return append(haystack, needle)

    elsif pOrder=ADD_SORT_UP then

        return sort(append(haystack, needle))

    elsif pOrder=ADD_SORT_DOWN then
...
public function remove_item(object needle, sequence haystack)
    integer lIdx = find(needle, haystack)
    if not lIdx then
        return haystack
    end if
...
--DEV temp, that )+1 should probably be +1), but needs testing
--/**/  return 9/0
--/*
...
--/* Phix: defined in builtins\premoveall.e
...
public function retain_all(object needles, sequence haystack)
    ...
    sequence result = haystack
    integer lp = length(haystack),
            np = 1
...
public function replace_all(sequence source, object olddata, object newdata)
    ...
    integer endpos = 1
    while 1 do
        integer startpos = match(olddata, source, endpos)
        if startpos=0 then exit end if
        endpos = startpos+length(olddata)-1
--/**/  source[startpos..endpos] = newdata                      --/*
...
public function project(sequence source, sequence coords)
    integer ls = length(source),
            lc = length(coords)
    sequence result = repeat(repeat(0, lc), ls)
    for i=1 to ls do
        for j=1 to lc do
...
public function build_list(sequence source, object transformer, integer singleton = 1, object user_data = {})
    ...
    for i=1 to length(source) do
        sequence x = {source[i], {i, length(source)}, user_data}
...
public function sim_index(sequence A, sequence B)
    ...
    atom accum_score = 0,
         pos_factor
    integer indx_a = 1, total_elems
    sequence used_A = repeat(0, length(A)),
             used_B = repeat(0, length(B))
--/**/integer skip = 0
--/**/while indx_a<=length(A) do                    --/*
        ...
        while indx_b<=length(B) do
            if equal(A[indx_a],B[indx_b]) then
                ...
                while 1 do
                    ...
                    indx_a += 1
                    indx_b += 1
                    if indx_a>length(A) or indx_b>length(B) then exit end if
                    if not equal(A[indx_a], B[indx_b]) then exit end if
                end while
--/**/          skip = 1 exit       --/*
                continue "DoA"      --*/
            end if
