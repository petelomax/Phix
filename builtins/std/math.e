...
--/* Not Phix (defined in auto-include psign.e)
...
public function ensure_in_list(object item, sequence list, integer default_idx=1)
    if length(list) = 0 then
        return item
    end if
    if find(item, list) = 0 then
        if default_idx>=1 and default_idx<=length(list) then
            return list[default_idx]
        else
            return list[1]
        end if
    end if
...
--/* not Phix (deinfed in the autoinclude file builtins/porall.e)
