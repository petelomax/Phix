...
--/*
include std/machine.e           -- (Not Phix)
include std/dll.e               -- (Not Phix)
include std/sequence.e          -- (Not Phix)
include std/get.e               -- (Not Phix)
include std/error.e             -- (Not Phix)
include std/types.e             -- (Not Phix)
--*/
...
--DEV (ifdef implemented completely differently in Phix)
--ifdef WIN32 then
...
function dt_time()
ifdef WIN32 then
    atom deltahi = 27111902,
        deltalow = 3577643008,
            ptra = allocate(8)
    c_proc(time_, {ptra})
    atom vallow = peek4u(ptra),
          valhi = peek4u(ptra+4)
    free(ptra)
    vallow -= deltalow
    valhi -= deltahi
...
function gmtime(atom pTime)

    atom timep = allocate(4)
    poke4(timep, pTime)

    atom tm_p = c_func(gmtime_, {timep})

    free(timep)

    sequence res = repeat(0, 9)
    integer n = 0
...
--/**/integer ly
--/**/  ly = remainder(year,4)=0
--/**/
--/**/  if not ly then return 0 end if
--/**/
...
--/**/           -remainder(year,3200)=0
--/**/           +remainder(year,80000)=0
--/**/  end if
--/*
sequence ly
...
function julianDayOfYear(sequence ymd) -- returns an integer
    integer {year, month, day} = ymd
...
function secondsToDateTime(atom seconds) -- returns a DateTime
    ...
    integer days = floor(seconds/DayLengthInSeconds)
    seconds = remainder(seconds, DayLengthInSeconds)

    integer hours = floor(seconds/3600)
    seconds -= hours*3600

    integer minutes = floor(seconds/60)
    seconds -= minutes*60
...
public function dateformat(datetime d, sequence pattern = "%Y-%m-%d %H:%M:%S")
...
public function parse(string val, fmt="%Y-%m-%d %H:%M:%S")
    integer fpos = 1, spos = 1, maxlen, rpos, ffp, ch, epos
    sequence res = {0,0,0,0,0,0}, got
