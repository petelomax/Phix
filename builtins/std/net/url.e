...
public function parse(sequence url, integer querystring_also=0)
    ...
    integer port
integer pos, at, password_colon,qs_start,first_slash,port_colon,port_end
    ...
    -- If we do not have a username, password, host or port, we have moved right into 
    -- the path area of the URL. Let's jump ahead
while 1 do  -- dummy loop
    if url[pos] != '/' then

        at = find('@', url)
        if at then
    
            password_colon = find(':', url, pos)
