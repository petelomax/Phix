--
-- Euphoria 2.5 (Phix compatible)
-- Euphoria(/Phix) Database System (EDS)
--
...
--    key value. Euphoria(/Phix) objects, key and data, are stored in a 
...
constant DB_MAJOR = 2, DB_MINOR = 5   -- database created with Euphoria v2.5 (/Phix)
...
integer dbinit  dbinit = 0

procedure db_init()
-- (collected here for Phix)
    dbinit = 1
    db_names = {}
    db_file_nums = {}
    db_lock_methods = {}
...
-- Compressed format of Euphoria(/Phix) objects on disk
--
-- First byte:
...
-- read a compressed Euphoria(/Phix) object from disk
...
--30/12/13: (return as a string if possible)
--/**/  s = repeat(' ',len)     --/* -- Phix
        s = repeat(0, len)      --*/ -- RDS
        for i=1 to len do
...
-- return the compressed representation of a Euphoria(/Phix) object 
...
--/**/global procedure db_dump(integer fn, integer low_level_too = 0)
--/*
...
atom a
    ...
    if magic!=DB_MAGIC then
        puts(fn, "This is not a Euphoria(/Phix) Database file\n")
        return
    end if
    major = get1()
    minor = get1()
    printf(fn, "Euphoria(/Phix) Database System Version %d.%d\n\n", {major, minor})
    tables = get4()
    safe_seek(tables)
    ntables = get4()
...
--/**/global function db_create(sequence path, integer lock_method = DB_LOCK_NO)
--/*
...
integer db
    ...
    if not dbinit then db_init() end if
...
--/**/global function db_open(sequence path, integer lock_method = DB_LOCK_NO)
--/*
...
--trace(1)
    magic = getc(db)
--DEV: BAD LOGIC: crash outright or return other than DB_OPEN_FAIL.... DB_BAD_MAGIC?
...
integer index
    ...
    db_names = db_names[1..index-1] & db_names[index+1..length(db_names)]
    db_file_nums = db_file_nums[1..index-1] & db_file_nums[index+1..length(db_file_nums)]
    db_lock_methods = db_lock_methods[1..index-1] & db_lock_methods[index+1..length(db_lock_methods)]
    current_db = -1
--31/7/15: (for memory leak checking)
...
-- result is DB_OPEN_FAIL or DB_OK
...
atom table, tables, size, nt, newsize, newtables
sequence remaining
atom records_ptr, index_ptr, name_ptr

--/**/if not string(name) then return DB_BAD_NAME end if
...
object testkey
    ...
    while lo<=hi do
        mid = floor((lo+hi)/2)
        testkey = key_value(key_pointers[mid])
--if mid=37 then
--      c = compare(key, testkey)
--end if
        c = compare(key, testkey)
--printf(1,"%s %s %s [%d]\n",{sprint(key), leg[c+2], sprint(testkey), mid})
        if c<0 then
            hi = mid-1
        elsif c>0 then
            lo = mid+1
...
--/**/include builtins\syswait.ew   -- system()
...
sequence new_path, old_path, table_list, record, chunk, cj
    ...
    for i=1 to length(table_list) do
        ...
        while r<=nrecs do
            ...
            for j=1 to chunk_size do
                cj = chunk[j]
                if db_insert(cj[1], cj[2])!=DB_OK then
